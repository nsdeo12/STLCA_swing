...
public class Dynmaps10_original extends PApplet{
PApplet parent;
...
Table table = loadTable("C:\\Users\\Nitesh\\Desktop\\boeing 747-8\\Input_original.csv", "header");
 //Table table = loadTable("M:\\templates\\Input.csv", "header");
  ...
  for (int k1=0; k1<rows; k1++) {
   ...
   if (loctime[0][k1]-loctime[0][k1+1]==0 && loctime[1][k1]-loctime[1][k1+1]==0 ) { //If next space not equal to the current then x-coordiante + 20 
      assign = assign;
    }
    else if (loctime[0][k1]-loctime[0][k1+1]==0  && loctime[1][k1]-loctime[1][k1+1]!=0 ) {
  ...
  for (int k2=0; k2<rows; k2++) {
    if (loctime[0][k2]-loctime[0][k2+1]==0)// if space same
    {
...
public void draw() {
 ...
 /* text("S1", 20, -5);
  ...
  stroke(205);
  strokeWeight((float) 0.1);
  for (int gv=0; gv<1300; gv=gv+10) {
    line(0, gv, 1300, gv);                //Grid Where 1000 there  a variable with duration summation
  }
  for (int gh=0; gh<1300; gh=gh+10) {
    line(gh, 0, gh, 1300);                //Grid
  }
...
textSize(3);
  ...
  textSize(20);
  // LEFT LABEL
  text ("IMPACT SPECTRUM", 1150, 10);
  textSize(8);
  text ("            0.0006 to 0.0008", 1150, 32);
  text ("            0.0004 to 0.0006", 1150, 47);
  text ("            0.0001 to 0.0004", 1150, 62);
  text ("        0.0004 to 0.02", 1150, 77);
  text ("    0.02 to 0.04", 1150, 92);
  text ("    0.04 to 0.06", 1150, 107);
  text ("  0.06 to 0.1", 1150, 122);
  text ("0.1 to 0.3", 1150, 137);
  text ("0.3 to 0.5", 1150, 152);
  text ("0.5 to 0.3", 1150, 167);
  text("0.5", 1137, 182);
...
{    
 ...
 // text("OVERALL", Vis+20, 340);
  text("Impact = " +   iovesum/10000,  1150,300);
  text("Confidence = " +ConfidencePro6*100+ " %", 1150,380);
  textSize(3);
...
{  //Impact Calc  OVERALL Phase
  for (int i=0; i<rows; i++)  
  {
    if (loctime[21][i]!=0) // OVERALL Phase Impact Calc
    {
     ...
     iovesum= (-1)* (ipart6+iass6+ipro6); //OVERALL Sum Impact
...
{ 
  ...
  if (ipart6c > 0)
  {
	  ...
	  if ((ipart6c)>=0.3){
		...
		ellipse(check,1025, 30,30);}
		//text(ipart6c, check, 1010);}
		 else if  (((ipart6c)>=0.1) && (ipart6c)<0.3){
		   fill(241,89,40);
		ellipse(check,1010, 30,30);}
		  else if  (((ipart6c)>=0.06) && (ipart6c)<0.1){
		   fill(241,89,40);
		ellipse(check,1010, 30,30);}
		 else if (((ipart6c)>=0.02) && (ipart6c)<0.03){
		   fill(251,175,63);
		ellipse(check,1010, 30,30);}
		    else if (((ipart6c)>=0.01) && (ipart6c)<0.02){
		   fill(247,236,46);
		ellipse(check,1010, 30,30);}
		    else if (((ipart6c)>=0.0004) && (ipart6c)<0.01){
		   fill(214,223,34);
		ellipse(check,1010, 30,30);}
		    else if (((ipart6c)>=0.0003) && (ipart6c)<0.0004){
		   fill(140,198,62);
		 ellipse(check,1010,15,15);}
		    else if (((ipart6c)>=0.0002) && (ipart6c)<0.0003){
		   fill(36,181,114);
		ellipse(check,1010, 15,15);}
		    else if ((ipart6c)<=0.0002){ 
		   fill(0,173,239);
		 ellipse(check,1010, 15,15);}
  }
  fill(0);
  textSize(12);
  text(ipart6c, check, 1000);
  text ( "Impact = " , 40, 1000);
  fill(194,153,106);
  arc(80, 1090, 30, 30, 0, (float) (0.85*6.2831) , PIE);
  fill(0);
  
  text(" Confidence = "+ 0.8503 * 100 + "%" , 40, 1070);
  fill(194,153,106);
  arc(160, 1090, 30, 30, 0, (float) (0.32*6.2831) , PIE);
  fill(0);
