...
public class tree_imp extends PApplet {
	PApplet parent;
	...
	public static void main(String _args[]){
		...
		PApplet.main(new String[]{"--present", "lca.tree_imp"});
		//PApplet.main( System.setProperty("java.awt.headless", "true"));
		}
	...
	int rowc()
	{
	Table table1 = loadTable("C:\\Save\\macroTree\\new.csv", "header");
		  ...
		  for (int i = 0; i < columns; i++) {
		    for (int j = 0; j < rows; j++) {
		      ...
		      { 
		        ...
		        pp[5][j] = row1.getFloat("2");
		      } else if (i==6) {
		        pp[6][j] = row1.getFloat("3");
		      } else if (i==7) {
		        pp[7][j] = row1.getFloat("4");
		      } else if (i==8) {
		        pp[8][j] = row1.getFloat("5");
		      }
		    }
		  }
	 ...
	 if(pp[1][i] == 0)
	  {
		...
		/* lsum = lsum +100;
		 ...
		 // ellipse(s-50 , y , r, r);
		 fill(0);
	  text(Partname[0][i], s+35,y);
	//  text("Impact = " + pp[3][i], s+50 , y  );
	  y=y+70;
	}
	  if (pp[1][i] == 2)
	  {
