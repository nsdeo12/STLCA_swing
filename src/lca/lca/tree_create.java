...
public class tree_create extends PApplet {
	
	PApplet parent;

	public tree_create(PApplet _parent){
		parent = _parent;
	}
	...
	public static void main(String _args[]){
		PApplet.main(new String[]{"--present", "lca.tree_create"});
	}
...
//	int rows = 55; // Row no 2 to Row no Last... n-1 rows from excel
	    ...
	    for (int j = 0; j < rows1; j++) {
	      ...
	      { 
	        ...
	        pp[5][j] = row1.getFloat("Part");
		        ...
		        pp[7][j] = row1.getFloat("Confidence");
		      } 
	    }
	  }
	...
	void tree_draw(float pp2)
	{
		...
		if(pp[1][i] == pp2)
		  ...
		  text("Confidence = " + pp[7][i]  + "%", s+100 , y+5  ); 
		  y=y+70;
		}
		}
	}
	void ellip(){
		...
		for (int i = 0; i < columns; i++){ 
		 ...
		 if(pp[1][i] == 0)
		  ...
		  text("Confidence = " + pp[7][i] + "%",  s+175 , y+15  ); 
		  y=y+70;
		}
		   ...
		   if(pp[1][i] == 3)
		  {
			  ...
			  text("Impact = " + pp[3][i], s+350 , y-10 );
			   text("Confidence = " + pp[7][i] + "%",  s+350 , y+15  ); 
			  y=y+70;  
		  }
		  if(pp[1][i] == 4)
		  {
			  ...
			  text("Impact = " + pp[3][i], s+600 , y-10 );
			   text("Confidence = " + pp[7][i] + "%",  s+500 , y+15  ); 
			  y=y+70;  
		  }
		  if(pp[1][i] == 5)
		  {
	...
	private float cal_conf(int part) {
		...
		val_conf= ((nze /tot_pro ) * ( pro_multinz / tot_imp )) + ((ze /tot_pro) * (pro_multiz));
